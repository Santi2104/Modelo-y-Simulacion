document.getElementById("ejecutarCola").addEventListener('click', function(e) {
    ...
    pintarTabla(convertir(tiempo), convertir(proxLlegada), convertir(proxFinServicio), q, ps, psStatus, convertir(proxDescanso), convertir(proxTrabajo))
    ...
    function pintarTabla(tiempo, proxLlegada, proxFinServicio, q, ps, psStatus, proxDescanso, proxTrabajo) {
        document.getElementById("tablaprueba").insertRow(-1).innerHTML =
            `<td>${tiempo}</td>
                       <td>${proxLlegada}</td>
                       <td>${proxFinServicio}</td>
                       <td>${q}</td>
                       <td>${ps}</td>
                       <td>${psStatus}</td>
                       <td>${proxDescanso}</td>
